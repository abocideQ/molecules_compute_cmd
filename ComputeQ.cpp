...
long double ComputeQ::sumQ()
{
  ...
  long double Q = sumQe();
  std::cout << "Q = " << Q << endl;
  record_string += "Q = " + DecimalUtils::to_string(Q) + " \n";
  return Q;
}
...
long double ComputeQ::sumQv()
{
  ...
  for (size_t i = 0; i < m_vec_Gv.size(); i++)
  {
    ...
    record_string += "*** exp(hcG(v)/ktvib) = " + DecimalUtils::to_string(ret_Qv) + " \n";
  }
...
long double ComputeQ::sumQj(float v)
{
  ...
  for (size_t i = 0; i < m_vec_Fj.size(); i++)
  {
    ...
    record_string += "--- gjexp(hcF(j)/ktrot) = " + DecimalUtils::to_string(ret_Qj) + " \n";
  }
  return ret_Qj;
}
